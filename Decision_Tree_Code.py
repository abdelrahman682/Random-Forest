...
class DecisionTreeREG:
    ...
    def info_gain(self , y, left_indices, right_indices):
        parent_impurity = self.mean_sqr_error(y)
        left_impurity = self.mean_sqr_error(y[left_indices])
        right_impurity = self.mean_sqr_error(y[right_indices])
        w_left = len(left_indices) / len(y)
        w_right = len(right_indices) / len(y)
        var_reduction = parent_impurity - w_left * left_impurity - w_right * right_impurity
    ...
    def fit(self, X, y, depth = 0):
        ...
        for i in range(num_feature):
            ...
            for j in unique_values:
                ...
                if len(left_indices) == 0 or len(right_indices) ==0:
                    continue 
                var_reduction = self.info_gain(y, left_indices , right_indices)
